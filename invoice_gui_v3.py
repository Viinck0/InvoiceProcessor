...
Llama Vision může:
- Otevírat a prohlížet si soubory
- Analyzovat více stránek PDF (až 5)
- Provádět vícekrokovou detailní analýzu
- Vracet log toho co viděla

Použití:
    python invoice_gui_v3.py

Závislosti:
    pip install customtkinter PyMuPDF ollama Pillow psutil
"""
...
# AI Agent konstanty
MAX_PDF_PAGES = 3  # Analyzuje první 3 stránky (kompromis mezi rychlostí a přesností)
ENABLE_ZOOM = True
ANALYSIS_STEPS = 3
...
# ─────────────────────────────────────────────
# Modul: AI Vision Agent s přímým přístupem
# ─────────────────────────────────────────────
class AIVisionAgent:
    """
    AI Agent s přímým přístupem k souborům.
    Může otevírat PDF, prohlížet si více stránek a provádět detailní analýzu.
    """
...
- Důvod rozhodnutí (stručné vysvětlení)
...
{{
    ...
    "step3_decision": {{
        ...
        "confidence": number,
        "reason": string,
        "analysis_log": ["co jsi viděl", "co jsi našel"]
    }}
}}
...
{{
    "step1_overview": {{"page_count": 1, "document_type": "poznámka", "has_invoice_header": false}},
    "step2_details": {{"sender_name": "", "recipient_name": "", "invoice_number": "", "issue_date": "", "due_date": "", "total_amount": "", "currency": ""}},
    "step3_decision": {{"is_invoice": false, "confidence": 0.9, "reason": "Dokument obsahuje pouze datum a jméno, chybí náležitosti faktury", "analysis_log": ["Vidím pouze jméno a datum", "Chybí hlavička FAKTURA", "Žádná částka ani údaje o firmě"]}}
}}
...
Zhodnoť dokument na obrázku:"""
    ...
    def __init__(self, model: Optional[str] = None):
        ...
        # Import knihoven na úrovni agenta
        try:
            import fitz
    ...
    def _get_file_info(self, file_path: Path) -> dict:
        ...
        return {
            ...
            'filesize': file_path.stat().st_size / 1024,
    ...
    def analyze(self, file_path: Path) -> Optional[dict]:
        ...
        while retry_count <= max_retries:
            try:
                ...
                if "memory layout cannot be allocated" in error_msg or "CUDA out of memory" in error_msg:
                    ...
                    # Uvolnit paměť před dalším pokusem
                    ...
                    if retry_count <= max_retries:
                        ...
                        # Znovu připravit obrázky pro další pokus
                        images = self._prepare_images(file_path)
                        if not images:
                            logger.warning(f"Nepodařilo se znovu připravit obrázky")
                            return None
                        continue
                    else:
        ...
        # Uvolnění paměti po úspěšném dokončení nebo vyčerpání pokusů
        if images is not None:
            del images
            images = None
        gc.collect()
...
# GUI Aplikace
# ─────────────────────────────────────────────
class InvoiceProcessorGUIV3(ctk.CTk):
    """GUI aplikace pro Invoice Processor v3 s AI Agentem."""

    def __init__(self):
        super().__init__()

        self.title("🧾 Invoice Processor v3 - AI Agent s přímým přístupem")
    ...
    def _setup_ui(self):
        ...
        subtitle_label = ctk.CTkLabel(
            header_frame,
            text="🤖 AI Agent s přímým přístupem k souborům - analyzuje více stránek PDF",
        ...
        columns = ("soubor", "odesilatel", "prijemce", "datum", "castka", "jistota", "stranek")
        ...
        self.results_tree.heading("stranek", text="Stran")

        self.results_tree.column("soubor", width=180)
        self.results_tree.column("odesilatel", width=140)
        self.results_tree.column("prijemce", width=140)
        self.results_tree.column("datum", width=90)
        self.results_tree.column("castka", width=90)
        self.results_tree.column("jistota", width=70)
        self.results_tree.column("stranek", width=60)
        ...
        self.log_text = ctk.CTkTextbox(output_frame, font=("Consolas", 11), height=100)
        ...
        # Status bar
        self.status_var = ctk.StringVar(value="Stav: Připraveno | AI Agent | Model: " + VISION_MODEL)
    ...
    def _start_processing(self):
        ...
        self.start_button.configure(state="disabled", text="⏳ AI Agent analyzuje...")
        self.cancel_button.configure(state="normal")
        self._clear_log()

        self.agent.model = self.model_name.get()
        self.status_var.set(f"Stav: AI Agent analyzuje | Model: {self.agent.model}")
    ...
    def _reset_ui(self):
        self.is_processing = False
        self.start_button.configure(state="normal", text="▶ Spustit AI Agent analýzu")
        self.cancel_button.configure(state="disabled")
        self._update_progress(0, "Připraveno")
        self.status_var.set(f"Stav: Připraveno | AI Agent | Model: {self.agent.model}")
    ...
    def _process_thread(self, source: str, target: str):
        """Hlavní processing thread s AI Agentem."""
        try:
            ...
            self._log(f"🤖 AI Agent verze s přímým přístupem k souborům")
            ...
            if not self.found_files:
                self.after(0, lambda: messagebox.showinfo("Info", "Nenalezeny žádné podporované soubory."))
            ...
            # Krok 2: AI Agent analýza
            self._log("🤖 Spouštím AI Agent analýzu...")
            self._log(f"   Model: {self.agent.model}")
            self._log(f"   MAX_IMAGE_SIZE: {MAX_IMAGE_SIZE}px")
            self._log(f"   MAX_PDF_PAGES: {MAX_PDF_PAGES} stránek")
            ...
            for idx, file_path in enumerate(self.found_files, start=1):
                ...
                if not is_invoice:
                    ...
                    self._log(f"  – {file_path.name}: Není faktura ({doc_type}) - {reason}")
                    continue
                ...
                # Zobrazení logu analýzy
                analysis_preview = " | ".join(invoice.analysis_log[:2]) if invoice.analysis_log else ""
                self._log(
                    f"  ✓ {file_path.name}: Faktura ({invoice.page_count} stran) "
                    f"od {invoice.sender_name[:20]} ({invoice.confidence:.0%})"
                )
                if analysis_preview:
                    self._log(f"     📝 {analysis_preview[:80]}...")
            ...
            if not self.filtered_invoices:
                self.after(0, lambda: messagebox.showinfo("Info", "Nebyly nalezeny žádné faktury."))
                self.after(0, self._reset_ui)
                return
